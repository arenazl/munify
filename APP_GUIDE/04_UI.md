# 03 - Layout, UI/UX y Mejores Prácticas
...
> **Referencia:** Basado en [Dashboard Design Patterns](https://dashboarddesignpatterns.github.io/patterns.html) y [mejores prácticas 2024](https://www.shaheermalik.com/blog/dashboard-design-15-best-practices-of-dashboard-design)

### 3.1 Estructura del Layout

> **⚠️ REGLA CRÍTICA:** El usuario y logout van en el **HEADER**, NUNCA en el sidebar. El sidebar es SOLO para navegación.

```
┌─────────────────────────────────────────────────────────────────────┐
│                           HEADER (Top Bar)                          │
│  ┌───────────────────────────────────────────────────────────────┐  │
│  │ [☰]  "Bienvenido, Juan"              [🎨 Paleta] [👤 Usuario] │  │
│  │                                       └─dropdown─┘└─dropdown─┘│  │
│  └───────────────────────────────────────────────────────────────┘  │
├──────────────┬──────────────────────────────────────────────────────┤
...
|------|-----------|------------|
| **Header izquierda** | Botón menú (móvil), mensaje bienvenida | - |
| **Header derecha** | Selector paleta, Menú usuario (avatar + dropdown con logout) | - |
| **Sidebar** | Logo, Links de navegación | ❌ NO usuario, ❌ NO logout |
| **Main content** | Título página, contenido, formularios, tablas | - |
...
const THEME_COLORS = [
  ...
  { name: 'Naranja Energía', value: 'orange', primary: '#ea580c' },
  ...
  return (
    <div className="min-h-screen bg-background flex">
      ...
      >
        ...
        {/* Navigation - SOLO links, NO usuario aquí */}
        ...
        {/* ⚠️ NO poner usuario/logout aquí - va en el header */}
      </aside>
                ...
                <DropdownMenuContent align="end">
                  {THEME_COLORS.map((color) => (
                    ...
                    >
                      ...
                      {theme === color.value && <span className="ml-auto">✓</span>}
                    </DropdownMenuItem>
                  ))}
                </DropdownMenuContent>
              </DropdownMenu>
...
## 4. Patrón ABM Obligatorio (CRÍTICO)

> **⚠️ REGLA CRÍTICA:** TODOS los ABMs (Alta, Baja, Modificación) de la aplicación DEBEN seguir este patrón. NO crear páginas separadas para agregar/editar.

### 4.1 Estructura de un ABM

```
┌─────────────────────────────────────────────────────────────────────┐
│  PÁGINA PRINCIPAL (Listado)              │  SIDE MODAL (Sheet)     │
│                                          │                         │
│  ┌─────────────────────────────────────┐ │  ┌─────────────────────┐│
│  │ Título         [+ Nueva Cuadrilla]  │ │  │ Nueva Cuadrilla     ││
│  └─────────────────────────────────────┘ │  │                     ││
│                                          │  │ [Formulario]        ││
│  ┌─────────────────────────────────────┐ │  │                     ││
│  │ 🔍 Buscar...  [Filtro ▼] [Filtro ▼] │ │  │ - Nombre            ││
│  └─────────────────────────────────────┘ │  │ - Especialidad      ││
│                                          │  │ - Zona              ││
│  ┌─────────────────────────────────────┐ │  │                     ││
│  │ Tabla con datos                     │ │  │ [Cancelar] [Guardar]││
│  │ ─────────────────────────────────── │ │  └─────────────────────┘│
│  │ 🔧 Cuadrilla Norte  | Alumbrado |✏️🗑│ │                         │
│  │ 🔧 Cuadrilla Sur    | Bacheo    |✏️🗑│ │    ◄── Se abre desde   │
│  │ 🔧 Cuadrilla Centro | Limpieza  |✏️🗑│ │        el listado      │
│  └─────────────────────────────────────┘ │                         │
└─────────────────────────────────────────────────────────────────────┘
```

### 4.2 Reglas del Patrón ABM

| Elemento | Implementación | ❌ NO hacer |
|----------|----------------|-------------|
| **Listado** | Página completa con tabla | Página vacía que solo lista IDs |
| **Crear nuevo** | Side modal (Sheet) | Página `/cuadrillas/nuevo` |
| **Editar** | Side modal (Sheet) | Página `/cuadrillas/:id/editar` |
| **Ver detalle** | Modal o expandir fila | Página separada (excepto si es complejo) |
| **Eliminar** | Confirmación en dialog | Eliminar sin confirmar |
| **Filtros** | En la misma página | Página de filtros separada |

### 4.3 Por qué Side Modals (NO páginas separadas)

- **Mejor UX**: No bloquean la vista del contenido principal
- **Más espacio**: Ideal para formularios largos
- **Consistencia**: Patrón moderno usado en apps como Notion, Linear
- **Contexto**: El usuario no pierde de vista el listado mientras edita
- **Navegación**: No se acumulan URLs en el historial
...
### 4.7 ABMs de la Aplicación (Todos con Side Modal)

| ABM | Página Listado | Sheet Component | Campos del Formulario |
|-----|----------------|-----------------|----------------------|
| **Categorías** | `/categorias` | `CategoriaSheet.tsx` | nombre, descripcion, icono, color, tiempo_resolucion, prioridad, activo |
...
> **Nota:** Los reclamos NO son un ABM tradicional porque tienen un flujo especial (crear → asignar → resolver). El formulario de nuevo reclamo puede ser página completa por su complejidad (mapa, fotos).

### 4.8 Checklist de Implementación ABM

Para cada ABM de la tabla anterior:

- [ ] Crear página de listado con tabla moderna (íconos, badges de estado)
- [ ] Crear componente Sheet para agregar/editar
- [ ] Implementar filtros en la misma página
- [ ] Botón "Nuevo" abre Sheet (no navega a otra página)
- [ ] Botón "Editar" en cada fila abre Sheet con datos precargados
- [ ] Botón "Eliminar" muestra dialog de confirmación
...
frontend/src/
├── lib/
│   └── themes.ts           # Definición de paletas
├── contexts/
...
> **⚠️ IMPORTANTE:** NO usar `@apply` con clases que usan variables CSS (como `@apply border-border` o `@apply bg-primary`). Tailwind no puede resolver estas clases en tiempo de compilación porque las variables CSS se evalúan en runtime. Usar CSS directo en su lugar.
...
/* ⚠️ USAR CSS DIRECTO - NO @apply con variables CSS */
@layer base {
  * {
...
```tsx
// Dentro del Layout.tsx - Sección del Header
...
### 6.8 Checklist de Implementación de Paletas
...
### 7.1 Patrón de Página de Listado
...
**Causa:** Usar `@apply` con clases que dependen de variables CSS (como `border-border`, `bg-primary`, `text-foreground`). Tailwind intenta resolver estas clases en tiempo de compilación, pero las variables CSS se evalúan en runtime.

**Solución:** Usar CSS directo en lugar de `@apply`:
...
**Solución:** Para componentes custom, usar CSS directo:
...
| Tipo de clase | ¿Usar @apply? | Ejemplo |
|---------------|---------------|---------|
| Clases estándar de Tailwind | ✅ Sí | `@apply flex items-center p-4` |
| Clases con variables CSS | ❌ No | `@apply bg-primary border-border` |
| Colores de Tailwind estándar | ✅ Sí | `@apply bg-blue-500 text-white` |
| Colores custom con HSL | ❌ No | `@apply bg-background text-foreground` |

### 11.4 Cuándo SÍ usar @apply
...
/* ✅ Colores fijos de Tailwind - también funciona */
.badge-pendiente {
...
> **⚠️ IMPORTANTE:** Este es el componente REAL usado en la aplicación. Todos los ABMs DEBEN usar este componente.

### 12.1 Ubicación y Estructura
...
- `ABMSheetFooter` - Footer estándar para modales
...
interface ABMPageProps {
  ...
  buttonLabel: string;     // Texto del botón "Nuevo X"
  onAdd: () => void;       // Handler del botón
...
export default function Categorias() {
  ...
  return (
    <ABMPage
      ...
      sheetOpen={sheetOpen}
      sheetTitle="Nueva Categoría"
      sheetContent={<FormularioCategoría />}
      sheetFooter={
        <ABMSheetFooter
...
|-----|---------|------------|
| Categorías | `pages/Categorias.tsx` | create/edit |
...
Las tarjetas tienen múltiples efectos:
...
El componente inyecta automáticamente los CSS necesarios:
...
**Shimmer effect** (brillo que pasa por el botón):
...
### 13.7 Clases de Animación Comunes
...
### 13.8 Best Practices de Animación

1. **Duraciones consistentes**: 200-300ms para micro-interacciones, 300-500ms para transiciones de página
...
> **IMPLEMENTADO:** El chat ya está funcionando con Ollama como provider por defecto.
...
│  │  - Genera contexto del sistema municipal            │    │
...
### 14.7 Personalización del System Prompt
...
- Cómo crear un nuevo reclamo
- Categorías disponibles: Baches, Alumbrado, Limpieza, Agua, Veredas, etc.
- Estados de reclamos: Nuevo, Asignado, En Proceso, Resuelto, Rechazado
- Tiempos estimados de resolución por categoría
- Información sobre las cuadrillas de trabajo

LIMITACIONES:
- No puedes crear reclamos directamente
- No puedes ver el estado de reclamos específicos
- Para consultas específicas, dirigir al usuario a la sección "Mis Reclamos"

TONO: Amable, profesional, conciso. Respuestas de máximo 3-4 oraciones."""
```
...
Importar en `main.tsx` después de `index.css`:
...
### 15.3 Clases de Animación Disponibles

| Categoría | Clases | Uso |
|-----------|--------|-----|
| **Fade** | `fade-in`, `fade-in-up` | Entrada suave de elementos |
| **Slide** | `slide-in-left`, `slide-in-right`, `slide-in-up`, `slide-in-down` | Entrada desde dirección |
...
| **Rotate** | `rotate-animation` | Rotación continua |
...
| **Botones** | `btn-gradient-primary`, `btn-gradient-success`, `btn-gradient-warning` | Botón con gradiente y hover |
...
### 15.8 Ejemplo de Botón con Gradiente
...
```css
:root {
  --animation-duration: 0.3s;  /* Más lento: 0.5s, Más rápido: 0.2s */
  --animation-timing: cubic-bezier(0.4, 0, 0.2, 1);  /* Ease estándar */
}
```

### 15.10 Checklist de Implementación
