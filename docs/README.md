...
| [`handoffs/`](handoffs/) | Cierres de sesión por fecha (`YYYY-MM-DD_titulo.md`). Último: `2026-09-03_zona-unica-y-catalogo-barrios.md` — la zona es del negocio (municipio → Zona única → barrios), la tabla `catalogo_barrios` con polígonos curada offline por provincia, y la semilla que ya no fabrica geografía: la lee y sólo genera los puntos de los reclamos. Antes: `2026-09-02_cartografia-offline-pbf.md` (la herramienta del PBF y `catalogo_geo_osm`) |
...
| **Retomar zonas / barrios / polígonos del mapa (Zona única + `catalogo_barrios`)** | [`handoffs/2026-09-03_zona-unica-y-catalogo-barrios.md`](handoffs/2026-09-03_zona-unica-y-catalogo-barrios.md) — **el más nuevo**: la decisión (la zona es del negocio), cómo se llena el catálogo de barrios por provincia, qué hace la semilla ahora, la cobertura COMPLETA de los 6 países (verificada con `merlo-2` en QA) y el JSON de faltantes (`backend/scripts/datos/faltantes_barrios.json`) que se cura con otras fuentes |
