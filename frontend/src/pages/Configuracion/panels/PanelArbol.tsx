...
export default function PanelArbol({ tramites, onNuevoTramite, onEditarTramite, onNuevaCategoria, onEditarCategoria }: PanelArbolProps) {
  ...
  /* "Expandir todo" abre TODOS los niveles de una (categorías y trámites):
     es la vista de control del canvas. El label alterna a "Contraer todo"
     cuando ya está todo abierto. La dependencia NO es un nivel del árbol:
     quién atiende cada trámite vive en Asignaciones. */
  const todosLosIds = useMemo(() => {
    const ids: string[] = [];
    tramites.forEach((cat: any) => {
      ids.push(`cat-${cat.id}`);
  ...
  const renderPrerrequisito = (req: any, idx: number) => (
    <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 16px', paddingLeft: '88px', background: 'var(--pl-surface-2)', borderBottom: '1px solid var(--pl-border)', minWidth: 0 }}>
  ...
  const renderTramite = (t: any) => {
    ...
    return (
      <React.Fragment key={id}>
        <div onClick={() => toggleExpand(id)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 16px', paddingLeft: '44px', background: isExpanded ? 'var(--pl-surface-2)' : 'var(--pl-surface)', borderBottom: '1px solid var(--pl-border)', cursor: 'pointer', minWidth: 0 }}>
        ...
        {isExpanded && (
          <div style={{ display: 'flex', alignItems: 'center', padding: '10px 16px', paddingLeft: '88px', background: 'var(--pl-surface-2)', borderBottom: '1px solid var(--pl-border)' }}>
  ...
  const renderCategoria = (cat: any) => {
    const id = `cat-${cat.id}`;
    const isExpanded = !!expanded[id];
    
    return (
      <React.Fragment key={id}>
        <div onClick={() => toggleExpand(id)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 16px', background: isExpanded ? 'var(--pl-surface-2)' : 'var(--pl-surface)', borderBottom: '1px solid var(--pl-border)', cursor: 'pointer', minWidth: 0 }}>
          <span style={{ display: 'grid', placeItems: 'center', width: '16px', flex: '0 0 16px' }}>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--pl-text-faint)" strokeWidth="2.4" strokeLinecap="round" style={{ transition: 'transform 200ms', transform: isExpanded ? 'rotate(90deg)' : 'rotate(0deg)' }}><path d="m9 6 6 6-6 6" /></svg>
          </span>
        ...
        {isExpanded && (
          <div style={{ display: 'flex', alignItems: 'center', padding: '12px 16px', paddingLeft: '52px', background: 'var(--pl-surface-2)', borderBottom: '1px solid var(--pl-border)' }}>
  ...
  return (
    <div style={{ marginTop: '0px' }}>
      ...
      {/* TREE CONTENT: la raíz son las CATEGORÍAS (la dependencia no es un nivel) */}
      <div style={{ background: 'var(--pl-surface)', border: '1px solid var(--pl-border)', borderRadius: '12px', overflow: 'hidden' }}>
        {tramites.map((cat: any) => renderCategoria(cat))}

        <div style={{ display: 'flex', alignItems: 'center', padding: '12px 16px', background: 'var(--pl-surface)', borderBottom: '1px solid var(--pl-border)' }}>
