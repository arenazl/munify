...
export default function Login() {
  ...
  // El acceso con Google se saco del login (dueño, 2026-09-15): en su lugar,
  // y en su mismo tamaño, esta el boton "No es tu municipio", que es lo que de
  // verdad hace falta en esta pantalla. Con `googleLogout` y el hook adentro,
  // ademas, el login cargaba el SDK de Google para un boton que casi nadie usa.
  ...
  useEffect(() => {
    ...
    if (!codigo || !nombre || municipioAjeno) {
      ...
      // Sin municipio que mostrar se limpia y se muestra el ACCESO GENERICO de
      // Munify, ACA MISMO. No se navega a ningún lado.
      //
      // Este renglón fue tres bugs distintos. Mandaba a la grilla de demos
      // hasta el 2026-09-03; después a `/super`, que es el login del super
      // admin y no el de un empleado municipal. Y el 2026-09-15, con la grilla
      // detrás del guard, la versión vieja hizo un BUCLE INFINITO que tumbó el
      // login de producción.
      //
      // No hace falta ir a ningún lado: para entrar no se elige municipio
      // antes. `handleSubmit` manda mail y clave, y el municipio sale de la
      // cuenta del usuario. Mandar a elegir era herencia de cuando la puerta de
      // la app era la vitrina de demos.
  ...
  // Sólo el NOMBRE es imprescindible para pintar. Sin `municipioCodigo` se
  // renderiza igual —es el acceso genérico de Munify— y lo único que no hay son
  // los accesos rápidos, que son de un municipio concreto.
  //
  // Antes esta condición pedía los dos y devolvía un SPINNER INFINITO a quien
  // llegara sin municipio recordado: ni entraba ni podía hacer nada. Es el
  // mismo encierro que el bucle, disfrazado de pantalla cargando.
  if (!municipioNombre) {
    return (
      ...
      <div className="min-h-screen w-full flex flex-col lg:flex-row text-white" style={{ background: fondoHero }}>
        ...
        <div className="lg:w-1/2 flex items-center justify-center px-6 py-6 sm:px-10 sm:py-8 lg:px-16 lg:py-6">
          <div className="w-full max-w-md">
            ...
            {/* LA SALIDA. Acá estaba "Continuar con Google", y en su lugar va lo que
                de verdad hace falta: poder salir del municipio equivocado.

                El municipio de arriba es el que quedó RECORDADO en este
                dispositivo, y no había forma de cambiarlo. Con la app instalada
                tampoco se puede editar la URL, así que el usuario quedaba
                encerrado en una demo ajena y la única salida era borrar el ícono
                y reinstalar — que es justo lo que no se le puede pedir a un
                cliente. Le pasó al dueño con Merlo el 2026-09-15.

                Se recarga con `replace` a propósito: el arranque vuelve a leer
                un localStorage limpio y no queda nada del municipio anterior ni
                en el tab, ni en el manifest, ni en el theme-color. */}
            {municipioCodigo && (
              <button
                type="button"
                onClick={() => {
